...
describe('AssignTeamsUseCase (FKs reais + transação)', () => {
    ...
    it('é idempotente quando players/teams já existem (não recria)', async () => {
        playerRepo.findOne
            .mockResolvedValueOnce({ id: 'p-alice' } as any)
            .mockResolvedValueOnce({ id: 'p-bob' } as any);
        teamRepo.findOne
            .mockResolvedValueOnce({ id: 't-1' } as any)
            .mockResolvedValueOnce({ id: 't-2' } as any);
